    ...
    for batch in chunks(image_ids, batch_size):
        ...
        for item in data:
            if "thumb_1024_url" not in item:
                tqdm.write(f"Missing URL for image {item['id']} for country of {country}")
...
def n_per_country(df, n, test_size, val_size, seed):
    ...
    for country, country_df in tqdm(df.groupby("country"), total=df["country"].nunique()):
        # shuffle per country, for random sampling
        country_df= country_df.sample(frac=1, random_state=seed).reset_index(drop=True)
        
        country_size = min(len(country_df), n)
        country_df = country_df.iloc[:country_size]
        country_df = obtain_image_urls(country_df, country)
        country_df = country_df.dropna(subset=["img_url"]).reset_index(drop=True)
    ...
    df_out = pd.concat(parts).sample(frac=1, random_state=seed).reset_index(drop=True)
...
def main():
    ...
    # save split data
    split_data.to_parquet("split_data.parquet", engine="fastparquet", index=False)
